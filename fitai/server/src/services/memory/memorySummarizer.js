 ...
 * this off the expensive path for small talk. Goes through the AI gateway
 * like every other AI task — budgets, breaker, provider cascade, schema
 * validation. Its fallback is SKIP, so an outage degrades to simply not
 * summarizing; system memories (memoryWriter) and the other tiers still work.
 */
const { queryAs } = require('../../db/userAccess');
const { generateMemorySummary } = require('../ai/aiOrchestrator');
...
async function summarizeAndStore({ userId, mode, userMessage, aiAnswer }) {
  ...
  let parsed;
  try {
    parsed = await generateMemorySummary({
      prompt: buildMemorySummaryPrompt({ userMessage, aiAnswer }),
      userId,
    });
  } catch (err) {
